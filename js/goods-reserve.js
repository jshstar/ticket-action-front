...
function initGoodsReservePage() {
    ...
    function getAuctionInfo() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var currentPrice = data['bidPrice'].toLocaleString();
                var startPrice = data['startPrice'].toLocaleString();
        ...
        sc = $('#seat-map').seatCharts({
            ...
            click: function () {
                if (this.status() == 'available') {
                    ...
                    let context = this;
                    if (previousSeat === 'available') {
                        addNormalSeatIntoCart(context, true);
                    } else {
                        $("#seat-info").load("/reservation/reserve-aside-normal-seat.html", function () {
                            $cart = $('#selected-seats');
                            $counter = $('#counter');
                            $total = $('#total');

                            addNormalSeatIntoCart(context, false);
                        });
                    }

                    previousSeat = this.status();
                    return 'selected';
                    ...
                    // 이전에 선택한 일반 예매 좌석이 있다면 취소 처리

                    $("#selected-seats").children("li").each(function () {
                    ...
                    $("#seat-info").load("/reservation/reserve-aside-auction.html", function () {
                        updateUserPointText();
                        currentViewAuctionLabel = context.settings.label;
                        // 경매 정보 받아오기
                        getAuctionInfo();
                        // SSE 설정
    ...
    // 좌석 선택 취소
    $(document).on('click', '#selected-seats .cancel-cart-item', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (const zone of placeInfo[currentPlaceId].zoneTypes) {
                    ...
                    remainingSeatDiv += `<span>${zoneGradeInfo[zone].gradeName}: ${remainSeatCount}석</span><br>`;
                    // .appendTo('#remaining-seat-info');
                }
            },
            error: function (jqXHR, textStatus) {
                errorAlert(jqXHR.responseJSON.message);
    ...
    function countdown() {
        if (auctionLeftTime > 0) {
            ...
            okAlert('경매가 종료되었습니다!').then((result) => {
                if (result) {
                    clearInterval(timerId);
                }
            });
    ...
    function addNormalSeatIntoCart(context, flag) {
        let seatPrice = context.data().price;
        $('<li>' + context.data().category + ' 좌석 # ' + context.settings.label + ': <b>' +
            seatPrice.toLocaleString('ko-KR') + '원</b> <a href="#" class="cancel-cart-item">[cancel]</a></li>')
            .attr('id', 'cart-item-' + context.settings.id)
            .data('seatId', context.settings.id)
            .appendTo($cart);

        const [zoneName, seatNumber] = context.settings.label.split('-');
        selectSeats.push(`${scheduleId}-${zoneGradeInfo[zoneName]['zoneGradeId']}-${seatNumber}`); // 선택한 좌석 장바구니에 넣기
        $counter.text(sc.find('selected').length + 1);

        if (flag) {
            let totalPrice = recalculateTotal(sc) + context.data().price;
            $total.text(totalPrice.toLocaleString());
        } else {
            $total.text(context.data().price.toLocaleString());
        }

        $("#remaining-seat-info").html(remainingSeatDiv);
    }
}
